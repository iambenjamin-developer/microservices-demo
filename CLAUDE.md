...
- **HTTP style (ADR 0007):** Catalog and the Gateway use minimal APIs (`IEndpoint`); Ordering and Inventory use MVC controllers deriving from `ApiControllerBase`, registered with `AddApiControllers()` (Inventory: one controller per slice, in the slice folder). Never mix styles inside a service. Failed `Result`s become `ToProblem()` / `Problem(error)`; FluentValidation runs through `WithRequestValidation<T>()` / the global `ValidationActionFilter`.
